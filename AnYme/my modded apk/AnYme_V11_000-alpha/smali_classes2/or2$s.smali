.class final enum Lor2$s;
.super Lor2;
.source ""


# annotations
.annotation system Ldalvik/annotation/EnclosingClass;
    value = Lor2;
.end annotation

.annotation system Ldalvik/annotation/InnerClass;
    accessFlags = 0x4008
    name = null
.end annotation


# direct methods
.method constructor <init>(Ljava/lang/String;I)V
    .locals 1

    const/4 v0, 0x0

    invoke-direct {p0, p1, p2, v0}, Lor2;-><init>(Ljava/lang/String;ILor2$k;)V

    return-void
.end method

.method private anythingElse(Lur2;Lyr2;)Z
    .locals 1

    const-string v0, "head"

    invoke-virtual {p2, v0}, Lyr2;->a(Ljava/lang/String;)Z

    invoke-virtual {p2, p1}, Lyr2;->a(Lur2;)Z

    move-result p1

    return p1
.end method


# virtual methods
.method process(Lur2;Lnr2;)Z
    .locals 10

    invoke-static {p1}, Lor2;->access$100(Lur2;)Z

    move-result v0

    const/4 v1, 0x1

    if-eqz v0, :cond_0

    invoke-virtual {p1}, Lur2;->a()Lur2$b;

    move-result-object p1

    invoke-virtual {p2, p1}, Lnr2;->a(Lur2$b;)V

    return v1

    :cond_0
    sget-object v0, Lor2$p;->a:[I

    iget-object v2, p1, Lur2;->a:Lur2$i;

    invoke-virtual {v2}, Ljava/lang/Enum;->ordinal()I

    move-result v2

    aget v0, v0, v2

    if-eq v0, v1, :cond_e

    const/4 v2, 0x2

    const/4 v3, 0x0

    if-eq v0, v2, :cond_d

    const/4 v2, 0x3

    const-string v4, "head"

    const-string v5, "html"

    if-eq v0, v2, :cond_4

    const/4 v2, 0x4

    if-eq v0, v2, :cond_1

    invoke-direct {p0, p1, p2}, Lor2$s;->anythingElse(Lur2;Lyr2;)Z

    move-result p1

    return p1

    :cond_1
    invoke-virtual {p1}, Lur2;->d()Lur2$f;

    move-result-object v0

    invoke-virtual {v0}, Lur2$h;->s()Ljava/lang/String;

    move-result-object v0

    invoke-virtual {v0, v4}, Ljava/lang/String;->equals(Ljava/lang/Object;)Z

    move-result v2

    if-eqz v2, :cond_2

    invoke-virtual {p2}, Lnr2;->w()Lorg/jsoup/nodes/h;

    sget-object p1, Lor2;->AfterHead:Lor2;

    :goto_0
    invoke-virtual {p2, p1}, Lnr2;->b(Lor2;)V

    goto/16 :goto_1

    :cond_2
    const-string v1, "body"

    const-string v2, "br"

    filled-new-array {v1, v5, v2}, [Ljava/lang/String;

    move-result-object v1

    invoke-static {v0, v1}, Lkr2;->a(Ljava/lang/String;[Ljava/lang/String;)Z

    move-result v0

    if-eqz v0, :cond_3

    invoke-direct {p0, p1, p2}, Lor2$s;->anythingElse(Lur2;Lyr2;)Z

    move-result p1

    return p1

    :cond_3
    invoke-virtual {p2, p0}, Lnr2;->a(Lor2;)V

    return v3

    :cond_4
    invoke-virtual {p1}, Lur2;->e()Lur2$g;

    move-result-object v0

    invoke-virtual {v0}, Lur2$h;->s()Ljava/lang/String;

    move-result-object v2

    invoke-virtual {v2, v5}, Ljava/lang/String;->equals(Ljava/lang/Object;)Z

    move-result v5

    if-eqz v5, :cond_5

    sget-object v0, Lor2;->InBody:Lor2;

    invoke-virtual {v0, p1, p2}, Lor2;->process(Lur2;Lnr2;)Z

    move-result p1

    return p1

    :cond_5
    const-string v5, "base"

    const-string v6, "basefont"

    const-string v7, "bgsound"

    const-string v8, "command"

    const-string v9, "link"

    filled-new-array {v5, v6, v7, v8, v9}, [Ljava/lang/String;

    move-result-object v6

    invoke-static {v2, v6}, Lkr2;->a(Ljava/lang/String;[Ljava/lang/String;)Z

    move-result v6

    if-eqz v6, :cond_6

    invoke-virtual {p2, v0}, Lnr2;->b(Lur2$g;)Lorg/jsoup/nodes/h;

    move-result-object p1

    invoke-virtual {v2, v5}, Ljava/lang/String;->equals(Ljava/lang/Object;)Z

    move-result v0

    if-eqz v0, :cond_f

    const-string v0, "href"

    invoke-virtual {p1, v0}, Lorg/jsoup/nodes/k;->c(Ljava/lang/String;)Z

    move-result v0

    if-eqz v0, :cond_f

    invoke-virtual {p2, p1}, Lnr2;->e(Lorg/jsoup/nodes/h;)V

    goto/16 :goto_1

    :cond_6
    const-string v5, "meta"

    invoke-virtual {v2, v5}, Ljava/lang/String;->equals(Ljava/lang/Object;)Z

    move-result v5

    if-eqz v5, :cond_7

    invoke-virtual {p2, v0}, Lnr2;->b(Lur2$g;)Lorg/jsoup/nodes/h;

    goto :goto_1

    :cond_7
    const-string v5, "title"

    invoke-virtual {v2, v5}, Ljava/lang/String;->equals(Ljava/lang/Object;)Z

    move-result v5

    if-eqz v5, :cond_8

    invoke-static {v0, p2}, Lor2;->access$200(Lur2$g;Lnr2;)V

    goto :goto_1

    :cond_8
    const-string v5, "noframes"

    const-string v6, "style"

    filled-new-array {v5, v6}, [Ljava/lang/String;

    move-result-object v5

    invoke-static {v2, v5}, Lkr2;->a(Ljava/lang/String;[Ljava/lang/String;)Z

    move-result v5

    if-eqz v5, :cond_9

    invoke-static {v0, p2}, Lor2;->access$300(Lur2$g;Lnr2;)V

    goto :goto_1

    :cond_9
    const-string v5, "noscript"

    invoke-virtual {v2, v5}, Ljava/lang/String;->equals(Ljava/lang/Object;)Z

    move-result v5

    if-eqz v5, :cond_a

    invoke-virtual {p2, v0}, Lnr2;->a(Lur2$g;)Lorg/jsoup/nodes/h;

    sget-object p1, Lor2;->InHeadNoscript:Lor2;

    goto/16 :goto_0

    :cond_a
    const-string v5, "script"

    invoke-virtual {v2, v5}, Ljava/lang/String;->equals(Ljava/lang/Object;)Z

    move-result v5

    if-eqz v5, :cond_b

    iget-object p1, p2, Lyr2;->b:Lwr2;

    sget-object v2, Lxr2;->ScriptData:Lxr2;

    invoke-virtual {p1, v2}, Lwr2;->d(Lxr2;)V

    invoke-virtual {p2}, Lnr2;->t()V

    sget-object p1, Lor2;->Text:Lor2;

    invoke-virtual {p2, p1}, Lnr2;->b(Lor2;)V

    invoke-virtual {p2, v0}, Lnr2;->a(Lur2$g;)Lorg/jsoup/nodes/h;

    goto :goto_1

    :cond_b
    invoke-virtual {v2, v4}, Ljava/lang/String;->equals(Ljava/lang/Object;)Z

    move-result v0

    if-eqz v0, :cond_c

    invoke-virtual {p2, p0}, Lnr2;->a(Lor2;)V

    return v3

    :cond_c
    invoke-direct {p0, p1, p2}, Lor2$s;->anythingElse(Lur2;Lyr2;)Z

    move-result p1

    return p1

    :cond_d
    invoke-virtual {p2, p0}, Lnr2;->a(Lor2;)V

    return v3

    :cond_e
    invoke-virtual {p1}, Lur2;->b()Lur2$c;

    move-result-object p1

    invoke-virtual {p2, p1}, Lnr2;->a(Lur2$c;)V

    :cond_f
    :goto_1
    return v1
.end method
